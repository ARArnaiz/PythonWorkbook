...
class ShelfFullError(Exception):
    """Raised when a book cannot be added due to capacity or width limits."""
    pass


class Book:
    def __init__(self, title: str, author: str, price: float, width: float):
        self.title = title
        self.author = author
        self.price = price
        self.width = width
...
class Shelf:
    def __init__(self, capacity: int, max_width: float):
    ...
    def add_books(self, *books: Book) -> None:
        for book in books:
            if len(self.books) >= self.capacity:
                raise ShelfFullError(f"Capacity of {self.capacity} reached; cannot add '{book.title}'.")
            if self.total_width() + book.width > self.max_width:
                raise ShelfFullError(f"Width limit exceeded; cannot add '{book.title}'.")
