CONST_BOOKS = [
    {"rank": 1, "author": "Gabriel García Márquez", "title": "Cien años de soledad",
     "en": "One Hundred Years of Solitude", "year": 1967, "country": "CO", "price": 17, "width": 2.5},
    {"rank": 2, "author": "Julio Cortázar", "title": "Rayuela", "en": "Hopscotch", "year": 1963, "country": "AR",
     "price": 18, "width": 3},
    {"rank": 3, "author": "Mario Vargas Llosa", "title": "La ciudad y los perros", "en": "The Time of the Hero",
     "year": 1963, "country": "PE", "price": 15, "width": 2.1},
    {"rank": 4, "author": "Carlos Fuentes", "title": "La muerte de Artemio Cruz", "en": "The Death of Artemio Cruz",
     "year": 1962, "country": "MX", "price": 16, "width": 2.2},
    {"rank": 5, "author": "Juan Rulfo", "title": "Pedro Páramo", "en": "Pedro Páramo", "year": 1955, "country": "MX",
     "price": 16, "width": 1.1},
    {"rank": 6, "author": "Mario Vargas Llosa", "title": "Conversación en La Catedral",
     "en": "Conversation in the Cathedral", "year": 1969, "country": "PE", "price": 18, "width": 3.3},
    {"rank": 7, "author": "Gabriel García Márquez", "title": "El coronel no tiene quien le escriba",
     "en": "No One Writes to the Colonel", "year": 1961, "country": "CO", "price": 14, "width": 1.8},
    {"rank": 8, "author": "José Donoso", "title": "El obsceno pájaro de la noche", "en": "The Obscene Bird of Night",
     "year": 1970, "country": "AR", "price": 19, "width": 3.0},
    {"rank": 9, "author": "Guillermo Cabrera Infante", "title": "Tres tristes tigres", "en": "Three Trapped Tigers",
     "year": 1967, "country": "CU", "price": 17, "width": 2.5},
    {"rank": 10, "author": "Alejo Carpentier", "title": "Los pasos perdidos", "en": "The Lost Steps", "year": 1953,
     "country": "CU", "price": 16, "width": 2.0},
    {"rank": 11, "author": "Carlos Fuentes", "title": "La región más transparente", "en": "Where the Air Is Clear",
     "year": 1958, "country": "MX", "price": 15, "width": 1.5},
    {"rank": 12, "author": "Mario Vargas Llosa", "title": "La casa verde", "en": "The Green House", "year": 1966,
     "country": "PE", "price": 16, "width": 2.2},
    {"rank": 13, "author": "Julio Cortázar", "title": "Bestiario", "en": "Bestiary (stories)", "year": 1951,
     "country": "AR", "price": 14, "width": 1.2},
    {"rank": 14, "author": "Gabriel García Márquez", "title": "El otoño del patriarca",
     "en": "The Autumn of the Patriarch", "year": 1975, "country": "CO", "price": 16, "width": 2.0},
    {"rank": 15, "author": "Augusto Roa Bastos", "title": "Yo el Supremo", "en": "I, the Supreme", "year": 1974,
     "country": "PA", "price": 20, "width": 2.5},
    {"rank": 16, "author": "Miguel-Ángel Asturias", "title": "El Señor Presidente", "en": "Mister President",
     "year": 1946, "country": "GU", "price": 15, "width": 1.8},
    {"rank": 17, "author": "Manuel Puig", "title": "El beso de la mujer araña", "en": "Kiss of the Spider Woman",
     "year": 1976, "country": "AR", "price": 15, "width": 2.0},
    {"rank": 18, "author": "José Lezama Lima", "title": "Paradiso", "en": "Paradiso", "year": 1966, "country": "CU",
     "price": 22, "width": 2.5},
    {"rank": 19, "author": "Alejo Carpentier", "title": "El siglo de las luces", "en": "Explosion in a Cathedral",
     "year": 1962, "country": "CU", "price": 17, "width": 2.0},
    {"rank": 20, "author": "Ernesto Sábato", "title": "Sobre héroes y tumbas", "en": "On Heroes and Tombs",
     "year": 1961, "country": "AR", "price": 18, "width": 2.0},
]


class Book:
    def __init__(self, title: str, author: str, price: float, width: int | float = 0.0):
        self.title = title
        self.author = author
        self.price = price
        self.width = width

    def __str__(self) -> str:
        return f"'{self.title}' by {self.author} at ${self.price}"


class Shelf:
    def __init__(self, capacity: int, width: int | float):
        self.books = []
        self.capacity = capacity
        self.maxwidth = width
        self.currentwidth = sum(book.width for book in self.books)


    def add_book(self, book: Book) -> None:
        if len(self.books) < self.capacity and self.currentwidth + book.width <= self.maxwidth:
            self.books.append(book)
            self.currentwidth += book.width
        else:
            raise ValueError(f"Shelf is full, no space left for this book: {book.title}.")

    def total_price(self) -> float:
        return sum(book.price for book in self.books)

    def has_book(self, title: str) -> bool:
        return any(book.title == title for book in self.books)


latinamerican_books = []
latbwidth = 0
for book in CONST_BOOKS:
    book_obj = Book(book["title"], book["author"], book["price"], book["width"])
    latbwidth += book["width"]
    latinamerican_books.append(book_obj)

print(len(latinamerican_books))
print(latbwidth)

# for book in latinamerican_books:
#     print(book)

shelf1 = Shelf(20, 50)
for book in latinamerican_books:
    shelf1.add_book(book)
print(f'Shelf 1 has {len(shelf1.books)} books for a total of ${shelf1.total_price()}')

for book in shelf1.books:
    print(book)

print(shelf1.has_book("El coronel no tiene quien le escriba"))
print(shelf1.has_book("El pozo"))

print(shelf1.currentwidth)
print(shelf1.maxwidth)